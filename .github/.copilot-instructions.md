...
**Learning resources** (`docs/learning/*.md`) → educational deep-dives explaining technical concepts from first principles (genetic algorithms, neural networks, RL basics, etc.)
...
- **`/docs/learning`** → educational deep-dives on technical concepts (genetic algorithms, neural networks, RL basics)

---

### `/docs/learning` — Educational Resources

**Purpose:** Teach technical concepts from first principles so anyone can understand the implementation without prior expertise.

**When to create a learning doc:**

When introducing ANY complex technical concept that requires specialized knowledge:
- Algorithms (genetic algorithms, pathfinding, etc.)
- Mathematical techniques (optimization, probability distributions)
- ML/RL concepts (neural networks, reward shaping, exploration-exploitation)
- Data structures (spatial indexing, graphs)
- Design patterns (observer, strategy, factory)

**Structure of a learning doc:**

1. **What:** One-sentence definition
2. **Why:** What problem does it solve?
3. **How:** Algorithm/formula/code with clear steps
4. **Worked Example:** Complete walkthrough with specific numbers
...
- Every formula must have variable definitions + plain-language intuition
- Every algorithm must have a concrete worked example with step-by-step execution
- Use ASCII diagrams for simple flows
- Code snippets should be < 20 lines with explanatory comments
- Compare alternatives (e.g., "tournament vs rank-based selection")
...
> One-sentence summary for complete beginners.
...
[What problem does it solve? What would be worse without it?]

## How It Works

### The Algorithm

[Pseudocode or clear steps]
